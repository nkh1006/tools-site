...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "DdayPage" });
  return {
    title: t("metaTitle"),
    ...
    alternates: {
      languages: {
        ko: "/dday-calculator",
        en: "/en/dday-calculator",
      },
    },
  };
}

export default async function DdayCalculatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("DdayPage");
