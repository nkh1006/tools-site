...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "DueDatePage" });
  return {
    title: t("metaTitle"),
    description: t("metaDescription"),
    alternates: {
      languages: {
        ko: "/due-date-calculator",
        en: "/en/due-date-calculator",
      },
    },
  };
}

export default async function DueDateCalculatorPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  ...
  const t = await getTranslations("DueDatePage");
